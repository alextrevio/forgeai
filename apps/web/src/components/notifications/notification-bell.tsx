  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="relative flex items-center justify-center rounded-lg p-2 text-gray-400 hover:bg-[#1e1e3a] hover:text-white transition-colors"
        title="Notifications"
      >
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-80 rounded-xl border border-[#27274a] bg-[#131320] shadow-2xl z-50 overflow-hidden">
          {/* Header */}
          <div className="flex items-center justify-between px-4 py-3 border-b border-[#27274a]">
            ...
            {safeArray(notifications).length === 0 ? (
              ...
              safeArray(notifications).slice(0, 20).map((n) => (
                <button
                  ...
                  className={cn(
                    "flex items-start gap-3 w-full px-4 py-3 text-left hover:bg-[#1e1e3a]/50 transition-colors border-b border-[#27274a]/50 last:border-b-0",
                    !n.read && "bg-[#6d5cff]/5"
                  )}
                >
