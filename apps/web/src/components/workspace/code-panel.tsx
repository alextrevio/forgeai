...
const MonacoEditor = dynamic(() => import("@monaco-editor/react"), {
  ...
  loading: () => (
    <div className="flex h-full items-center justify-center bg-[#0A0A0A]">
      <span className="text-xs text-[#888888]">Loading editor...</span>
    </div>
  ),
});
...
function getFileIcon(name: string) {
  ...
  if (name.endsWith(".md")) return <FileText className="h-3.5 w-3.5 text-[#888888]" />;
  ...
  return <File className="h-3.5 w-3.5 text-[#888888]/60" />;
}
  ...
  return (
    <div>
      <button
        ...
        className={cn(
          "flex w-full items-center gap-1.5 px-2 py-[3px] text-[11px] hover:bg-[#1A1A1A] transition-all duration-100 file-slide-in",
      ...
      >
        {isDir ? (
          <>
            {expanded ? <ChevronDown className="h-3 w-3 text-[#888888]/60 shrink-0" /> : <ChevronRight className="h-3 w-3 text-[#888888]/60 shrink-0" />}
        ...
        <span className="truncate text-[#EDEDED]/80">{node.name}</span>
        ...
        <div className="flex items-center gap-1 border-b border-[#2A2A2A] px-3 py-1.5 bg-[#0A0A0A]">
          {breadcrumb.map((part, i) => (
            <span key={i} className="flex items-center gap-1">
              {i > 0 && <ChevronRight className="h-3 w-3 text-[#888888]/30" />}
              <span className={cn(
                "text-[11px]",
                i === breadcrumb.length - 1 ? "text-[#EDEDED] font-medium" : "text-[#888888]"
              )}>{part}</span>
            </span>
          ))}
        </div>
      ...
      <PanelGroup direction="horizontal" className="flex-1">
        ...
        <Panel defaultSize={28} minSize={15} maxSize={45}>
          <div className="h-full overflow-y-auto border-r border-[#2A2A2A] bg-[#0A0A0A]">
            <div className="px-3 py-2 text-[10px] font-semibold text-[#888888]/60 uppercase tracking-widest">Explorer</div>
            {safeArray(fileTree).length === 0 ? (
              <div className="px-3 py-4 text-[11px] text-[#888888]/40 text-center">No hay archivos</div>
            ) : (
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        file.path === activeFilePath
                          ...
                          : "text-[#888888] hover:text-[#EDEDED] hover:bg-[#111111]/50"
                      )}
                    ...
                    >
                      ...
                      <button
                        ...
                        className="ml-1 rounded p-0.5 hover:bg-[#1A1A1A] transition-colors opacity-0 group-hover:opacity-100"
                      >
                        <X className="h-3 w-3" />
                      </button>
                    </div>
              ...
              {activeFilePath ? (
                ...
                <div className="flex h-full items-center justify-center bg-[#0A0A0A]">
                  <div className="text-center">
                    <Code2 className="h-8 w-8 text-[#888888]/20 mx-auto mb-3" />
                    <p className="text-[11px] text-[#888888]/40">Selecciona un archivo para editar</p>
                  </div>
                </div>
              )}
            </div>
